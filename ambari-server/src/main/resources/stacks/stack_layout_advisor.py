...
class StackLayoutAdvisor(object):
  """
  Abstract class implemented by all stack layout advisors. Stack layout advisors advise on stack specific questions.

  Currently stack layout advisors provide following abilities:
...
class DefaultStackLayoutAdvisor(StackLayoutAdvisor):
  """
  Default stack layout advisor implementation.
  ...
  def isMasterComponentWithMultipleInstances(self, component):
    return self.getComponentAttribute(component, "setup_ha")
  ...
  def getMinComponentCount(self, component):
    min = self.getComponentAttribute(component, "ideal_minimum")
    if min is not None:
      return min
    return 1
  ...
  def getHostForComponent(self, component, hostsList):
    ...
    if len(hostsList) != 1:
      scheme = self.getComponentAttribute(component, "layout")
