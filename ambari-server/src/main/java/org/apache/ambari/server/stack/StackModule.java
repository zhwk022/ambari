...
public class StackModule extends BaseModule<StackModule, StackInfo> implements Validable {
  ...
  /**
   * Merge the stack one of its linked extensions.
   *
   * @param allStacks      all stacks in stack definition
   * @param commonServices all common services specified in the stack definition
   * @param parentVersion  version of the stacks parent
  ...
  private void mergeStackWithExtension(
    ...
    //mergeConfigurations(extension, allStacks, commonServices, extensions);
    //mergeRoleCommandOrder(extension);

    // grab stack level kerberos.json from parent stack
    /*if (stackInfo.getKerberosDescriptorFileLocation() == null) {
      stackInfo.setKerberosDescriptorFileLocation(extension.getModuleInfo().getKerberosDescriptorFileLocation());
    }
  ...
  private void mergeServiceWithExtensionServiceParent(
    ...
    if (extension == null || !extension.isValid()) {
      setInvalid(extension, "The service '" + serviceInfo.getName() + "' in stack '" + stackInfo.getName() + ":"
          + stackInfo.getVersion() + "' extends a non-existent service: '" + parent + "'");
    } else {
      ...
      if (parentService == null || !parentService.isValid()) {
        setInvalid(parentService, "The service '" + serviceInfo.getName() + "' in stack '" + stackInfo.getName() + ":"
            + stackInfo.getVersion() + "' extends a non-existent service: '" + parent + "'");
      }
      else
  ...
  private void setInvalid(Validable module, String error) {
    setValid(false);
    stackInfo.setValid(false);
    if (module == null) {
      setErrors(error);
      ...
      setErrors(module.getErrors());
      stackInfo.setErrors(module.getErrors());
    }
  }
  ...
  private void mergeServiceWithStackServiceParent(
    ...
    if (baseStack == null) {
      throw new AmbariException("The service '" + serviceInfo.getName() + "' in stack '" + stackInfo.getName() + ":"
          + stackInfo.getVersion() + "' extends a service in a non-existent extension: '" + baseStackKey + "'");
    }
  ...
  private void mergeRoleCommandOrder(StackModule parentStack) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Role Command Order for " + stackInfo.getName() + "-" + stackInfo.getVersion() +
        " parent " + stackInfo.getParentStackVersion());
      stackInfo.getRoleCommandOrder().printRoleCommandOrder(LOG);
    }
  }

  /**
   * Merge role command order with the extension
   *
   * @param extension    linked extension
   */
  private void mergeRoleCommandOrder(ExtensionModule extension) {
    if (extension.getModuleInfo().getRoleCommandOrder() == null)
      return;

    stackInfo.getRoleCommandOrder().merge(extension.getModuleInfo().getRoleCommandOrder(), true);
    if (LOG.isDebugEnabled()) {
      LOG.debug("Role Command Order for " + stackInfo.getName() + "-" + stackInfo.getVersion() +
