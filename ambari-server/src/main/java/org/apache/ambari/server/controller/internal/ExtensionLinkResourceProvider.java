...
public class ExtensionLinkResourceProvider extends AbstractControllerResourceProvider {
  ...
  public RequestStatus updateResources(Request request, Predicate predicate)
    ...
    if (predicate == null) {
      List<ExtensionLinkEntity> entities = dao.findAll();
      for (ExtensionLinkEntity entity : entities) {
        try {
          getManagementController().updateExtensionLink(entity);
          updated = true;
        } catch (AmbariException e) {
          throw new SystemException(e.getMessage(), e);
        }
      ...
      for (Map<String, Object> propertyMap : getPropertyMaps(predicate)) {
        ExtensionLinkRequest extensionLinkRequest = getRequest(propertyMap);
        try {
    ...
    //Need to reread the stacks/extensions directories so the latest information is available
	//This also regenerates the stacks' services folders for extension services
    try {
      getManagementController().updateStacks();
    } catch (AmbariException e) {
      throw new SystemException(e.getMessage(), e);
